...
class Tool:
    def __init__(self, x, y, toolType, attrs, brand, name):
        ...
        if self.toolType == "Trailer":
            self.fullImg = pg.image.load(self.anims['full']).convert_alpha()
            self.fullImg = pg.transform.scale(self.fullImg, (self.fullImg.get_width()*2/ZOOM, self.fullImg.get_height()*2/ZOOM))
        self.originalImage = pg.image.load(self.anims[self.anims['default']]).convert_alpha()
...
class Tractor:
    def __init__(self, x, y, attrs, brand, name):
        ...
        self.originalImage = pg.image.load(self.anims['normal']).convert_alpha()
...
class Header:
    def __init__(self, x, y, attrs, brand, name):
        ...
        self.defaultFuel = attrs['fuel']
        self.fuel = self.defaultFuel
        self.originalImage = pg.image.load(self.anims['pipeIn']).convert_alpha()
    ...
    def drawBuyMenu(self, screen, money, xp, clicked):
        if self.shopImgs == []:
            ...
            for img in list(self.shopItems["anims"].keys()):
                ...
                self.shopImgs.append(pg.transform.scale_by(pg.image.load(self.shopItems["anims"][img]).convert_alpha(), 3/ZOOM))
...
class Window:
    ...
    def _saveGame(self):
        ...
        open('save.json', 'w').write(json.dumps(saveData))
